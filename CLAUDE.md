...
## Build
- When building Maven projects, always use `mvn package` (not `mvn compile`, not `mvn test`). This ensures tests run and the artifact is assembled.
...
- When making code changes, automatically increment the semantic version in `pom.xml` based on the nature of the change, and do this **before** running any build so the version is baked into the artifact. This applies to **every** code change without exception — including small tweaks, default value changes, log additions, and single-line fixes:
  - **Patch** (x.y.Z): Bug fixes, minor corrections, log additions, default value changes, small tweaks

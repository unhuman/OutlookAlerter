 ...
 */
public class MacScreenFlasher implements ScreenFlasher {
    ...
    public static void registerOverlayWindows(List<JFrame> windows) {
    ...
    public static void setOnFlashReady(Runnable callback) {
    ...
    public void flashMultiple(List<CalendarEvent> events) {
        ...
        try {
            ...
            LogManager.getInstance().error(LogCategory.ALERT_PROCESSING,
                "MacScreenFlasher: EDT invocation error during window creation: " + (ite.getCause() != null ? ite.getCause().getMessage() : "unknown"));
        ...
        try {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                labelContent.append("<div style='margin-bottom: 10px;'>")
                    .append(HtmlUtil.escapeHtml(event.getSubject()));

                // Add location if it exists - show domain for URLs, full text for physical locations
                if (event.getLocation() != null && !event.getLocation().isEmpty()) {
                    String displayLocations = getMeetingLocations(event.getLocation());
                    if (displayLocations != null && !displayLocations.isEmpty()) {
    ...
    private Color getAlertColor() {
        try {
            ...
            if (config != null) {
                ...
                return (hexColor != null && !hexColor.isEmpty()) ? Color.decode(hexColor) : Color.RED;
            }
            return Color.RED;
        } catch (Exception e) {
    ...
    private Color getAlertTextColorWithOpacity() {
        try {
            ...
            if (config != null) {
                ...
                return (hexColor != null && !hexColor.isEmpty()) ? Color.decode(hexColor) : Color.WHITE;
            }
            return Color.WHITE;
        } catch (Exception e) {
    ...
    private String getMeetingLocations(String location) {
        ...
        for (String singleLocation : locations) {
            ...
            if (processedLocation != null && !processedLocation.isEmpty()) {
        ...
        return String.join("; ", processedLocations);
    }

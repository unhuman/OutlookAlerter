    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("--config") && i < args.length - 1) {
                ...
                consoleMode = true;
            } else if (args[i].equals("--timezone") && i < args.length - 1) {
        ...
        if (consoleMode) {
            ...
            try {
                ...
                LogManager.getInstance().warn(LogCategory.GENERAL, "Could not set system look and feel: " + e.getMessage());
            }
            
            // Start the UI application
            final String finalConfigPath = configPath;
            final String finalTimezoneOverride = timezoneOverride;
            SwingUtilities.invokeLater(() -> {
                OutlookAlerterUI app = new OutlookAlerterUI(finalConfigPath);
                
                // Apply timezone override if specified
                if (finalTimezoneOverride != null) {
                    app.setTimezone(finalTimezoneOverride);
                }
